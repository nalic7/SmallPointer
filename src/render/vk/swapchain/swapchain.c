...
//	if (m_vksurfacetransformflagbitskhr == VK_SURFACE_TRANSFORM_ROTATE_90_BIT_KHR || m_vksurfacetransformflagbitskhr == VK_SURFACE_TRANSFORM_ROTATE_270_BIT_KHR)
	...
	{
		vksc_vkextent2d.width = _sf_height;
		vksc_vkextent2d.height = _sf_width;
//		m_width = vksc_vkextent2d.width;
//		m_height = vksc_vkextent2d.height;
	}
	else
	{
		vksc_vkextent2d.height = _sf_height;
		vksc_vkextent2d.width = _sf_width;
	}
...
//	}
	...
	VK_makeImageView(vk_device, vkimage_depth, _VK_DEPTH_FORMAT, VK_IMAGE_ASPECT_DEPTH_BIT, 1, &vkimageview_depth)
	...
	VK_makeImageView(vk_device, vkimage_color, _VK_COLOR_FORMAT, VK_IMAGE_ASPECT_COLOR_BIT, 1, &vkimageview_color)

	for (uint32_t i = 0; i < swapchain_image; ++i)
	{
		VK_makeImageView(vk_device, vksc_vkswapchainkhr_vkimage_p[i], _VK_COLOR_FORMAT, VK_IMAGE_ASPECT_COLOR_BIT, 1, &vksc_vkswapchainkhr_vkimageview_p[i])
		VK_makeFramebuffer
		(
			vk_device,
			((VkImageView[])
