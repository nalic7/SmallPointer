...
static void re_sc()
{
	...
	vkQueueWaitIdle(Pinfo->Pvkqueue[Pinfo->gp]);
	vkQueueWaitIdle(Pinfo->Pvkqueue[Pinfo->sf]);
...
int smpt_rd_vk_cmdMloop(void *P)
{
	...
	while (!(smpt_sfUstate & SMPT_SFuS_EXIT))
	{
		...
		vkQueueSubmit(Pinfo->Pvkqueue[Pinfo->gp], 1, &image_vksubmitinfo, Pvkfence[smpt_rd_vk_swcUframe]);
		vkQueuePresentKHR(Pinfo->Pvkqueue[Pinfo->sf], &vkpresentinfokhr);
