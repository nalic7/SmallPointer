...
void vk_makeSwapchain(VkSharingMode vksharingmode)
{
	...
	VkDevice vkdevice = m_vkdevice_p[m_device];
		...
		vkGetPhysicalDeviceSurfaceFormatsKHR(vkphysicaldevice, m_vksurfacekhr, &surface_format, VK_NULL_HANDLE);
		VkSurfaceFormatKHR *vksurfaceformatkhr_p = malloc(surface_format * sizeof(VkSurfaceFormatKHR));
		vkGetPhysicalDeviceSurfaceFormatsKHR(vkphysicaldevice, m_vksurfacekhr, &surface_format, vksurfaceformatkhr_p);
		for (uint32_t i = 0; i < surface_format; ++i)
		{
			VkSurfaceFormatKHR vksurfaceformatkhr = vksurfaceformatkhr_p[i];
			info("i %d", i)
			info("format %d", vksurfaceformatkhr.format)
			info("colorSpace %d", vksurfaceformatkhr.colorSpace)
		}

		free(vksurfaceformatkhr_p);

		uint32_t surface_present;
		vkGetPhysicalDeviceSurfacePresentModesKHR(vkphysicaldevice, m_vksurfacekhr, &surface_present, VK_NULL_HANDLE);
		VkPresentModeKHR *vkpresentmodekhr_p = malloc(surface_present * sizeof(VkPresentModeKHR));
		vkGetPhysicalDeviceSurfacePresentModesKHR(vkphysicaldevice, m_vksurfacekhr, &surface_present, vkpresentmodekhr_p);
		for (uint32_t i = 0; i < surface_present; ++i)
		{
			info("VkPresentModeKHR %d", vkpresentmodekhr_p[i])
		}
	...
	vkresult = vkCreateSwapchainKHR(vkdevice, &vkswapchaincreateinfokhr, VK_NULL_HANDLE, &m_vkswapchainkhr);
	if (vkresult != VK_SUCCESS)
	{
		error("vkCreateSwapchainKHR %d", vkresult)
	}

	vkGetSwapchainImagesKHR(vkdevice, m_vkswapchainkhr, &swapchain_image, VK_NULL_HANDLE);

	#ifdef NALI_VK_INFO_SC
		info("vksurfaceformatkhr_image %d", swapchain_image)
	#endif
	...
	vkGetSwapchainImagesKHR(vkdevice, m_vkswapchainkhr, &swapchain_image, m_vkswapchainkhr_vkimage_p);
	...
	for (uint32_t i = 0; i < swapchain_image; ++i)
	{
		...
		vk_makeFrameBuffer(m_device, (VkImageView[]){vkimageview_color, vkimageview_depth, m_vkswapchainkhr_vkimageview_p[i]}, 3, &m_vkrenderpass, &m_vkswapchainkhr_vkframebuffer_p[i]);
	}
}
