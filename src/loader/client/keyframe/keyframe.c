...
void lckf_set()
{
	...
	{
		lckf_keyframe_bl_p[l_0] = *(uint8_t *)(NALI_LB_CACHE_P_D_P + NALI_LB_CACHE_P_D_BL_P[1]);
		NALI_LB_CACHE_P_D_BL_P[1] += sizeof(uint8_t);
		lckf_keyframe_p[l_0] = malloc(lckf_keyframe_bl_p[l_0] * sizeof(keyframe));

		for (uint32_t l_1 = 0; l_1 < lckf_keyframe_bl_p[l_0]; ++l_1)
		{
			lckf_keyframe_p[l_0][l_1].keyframe = *(uint8_t *)(NALI_LB_CACHE_P_D_P + NALI_LB_CACHE_P_D_BL_P[1]);
			NALI_LB_CACHE_P_D_BL_P[1] += sizeof(uint8_t);

			lckf_keyframe_p[l_0][l_1].bone_bl = *(uint8_t *)(NALI_LB_CACHE_P_D_P + NALI_LB_CACHE_P_D_BL_P[1]);
			NALI_LB_CACHE_P_D_BL_P[1] += sizeof(uint8_t);
			...
			{
				lckf_keyframe_p[l_0][l_1].bone_p[l_2] = *(uint8_t *)(NALI_LB_CACHE_P_D_P + NALI_LB_CACHE_P_D_BL_P[1]);
				NALI_LB_CACHE_P_D_BL_P[1] += sizeof(uint8_t);

				lckf_keyframe_p[l_0][l_1].animation_s_p[l_2] = malloc(sizeof(float) * 3);
				memcpy(lckf_keyframe_p[l_0][l_1].animation_s_p[l_2], NALI_LB_CACHE_P_D_P + NALI_LB_CACHE_P_D_BL_P[1], sizeof(float) * 3);
				NALI_LB_CACHE_P_D_BL_P[1] += sizeof(float) * 3;

				lckf_keyframe_p[l_0][l_1].animation_r_p[l_2] = malloc(sizeof(float) * 4);
				memcpy(lckf_keyframe_p[l_0][l_1].animation_r_p[l_2], NALI_LB_CACHE_P_D_P + NALI_LB_CACHE_P_D_BL_P[1], sizeof(float) * 4);
				NALI_LB_CACHE_P_D_BL_P[1] += sizeof(float) * 4;

				lckf_keyframe_p[l_0][l_1].animation_t_p[l_2]= malloc(sizeof(float) * 3);
				memcpy(lckf_keyframe_p[l_0][l_1].animation_t_p[l_2], NALI_LB_CACHE_P_D_P + NALI_LB_CACHE_P_D_BL_P[1], sizeof(float) * 3);
				NALI_LB_CACHE_P_D_BL_P[1] += sizeof(float) * 3;
			}
		}
	}
