void t_3d()
{
	...
	for (uint8_t l_0 = 0; l_0 < vksc_image; ++l_0)
	{
		for (uint8_t l_1 = 0; l_1 < lcm_ds_bl; ++l_1)
		{
			...
			NALI_D_INFO("vkMapMemory %d", vkMapMemory(vkdevice, lcm_vkdevicememory_p[l_0 + l_1 * vksc_image], 0, vkdevicesize, 0, &lcm_vkbuffer_mp[(l_0 + l_1 * vksc_image)]))
		}
	}

	VkWriteDescriptorSet vkwritedescriptorset_p[NALI_LCS_D_SIZE * vksc_image * lcm_ds_bl];
	VkDescriptorBufferInfo vkdescriptorbufferinfo_p[3 + 2 * vksc_image * lcm_ds_bl];
	lcs_s_bl = NALI_TEST_map_p[0] - 1;
	lcs_s_p = realloc(lcs_s_p, sizeof(lcs_s) * lcs_s_bl);
	//.i ds == 1
	//! need free
	VkDescriptorSet *vkdescriptorset_p = malloc(sizeof(VkDescriptorSet) * vksc_image * lcm_ds_bl);
	for (uint8_t l_0 = 0; l_0 < vksc_image * lcm_ds_bl; ++l_0)
	{
		vkds_make(vk_device, lcs_vkdescriptorpool, &lcs_vkdescriptorsetlayout, 1, vkdescriptorset_p + l_0);
	}
	...
	vkUpdateDescriptorSets(vkdevice, NALI_LCS_D_SIZE * vksc_image * lcm_ds_bl, vkwritedescriptorset_p, 0, VK_NULL_HANDLE);
...
//		*(float *)(lcm_vkbuffer_mp[vksc_buffer_frame + NALI_TEST_ds * vksc_image] + 4 * sizeof(float) + sizeof(float) * 4 * 3 * 2 + 2 * sizeof(float)) = 0;
	...
	vkFlushMappedMemoryRanges(vkdevice, 1, &(VkMappedMemoryRange)
	{
		...
		.memory = lcm_vkdevicememory_p[vksc_buffer_frame],
		.offset = 0,
...
void t_3d_buffer()
{
	...
	vksc_buffer_frame = (vksc_buffer_frame + 1) % vksc_image;
	if (NALI_TEST_m < lcp_joint_count_bl)
	{
		//apply rgba
		for (uint8_t l_0 = 0; l_0 < 4; ++l_0)
...
//	NALI_D_LOG("t_s_kf %f", t_s_kf);
	...
	for (uint8_t l_0 = 0; l_0 < l_lckf.bone_bl; ++l_0)
	{
		...
		{
			NALI_M_LERP(((float *)(lcm_vkbuffer_mp[vksc_buffer_frame + NALI_TEST_ds * vksc_image] + 4 * sizeof(float) + l_lckf.bone_p[l_0] * sizeof(float) * 4 * 3))[l_3], l_lckf.s_p[l_0][l_3], kff);
			NALI_M_LERP(((float *)(lcm_vkbuffer_mp[vksc_buffer_frame + NALI_TEST_ds * vksc_image] + 4 * sizeof(float) + l_lckf.bone_p[l_0] * sizeof(float) * 4 * 3 + sizeof(float) * 4 * 2))[l_3], l_lckf.t_p[l_0][l_3], kff);
		}

		for (uint8_t l_3 = 0; l_3 < 4; ++l_3)
		{
			NALI_M_LERP(((float *)(lcm_vkbuffer_mp[vksc_buffer_frame + NALI_TEST_ds * vksc_image] + 4 * sizeof(float) + l_lckf.bone_p[l_0] * sizeof(float) * 4 * 3 + sizeof(float) * 4))[l_3], l_lckf.r_p[l_0][l_3], kff);
		}
	}
